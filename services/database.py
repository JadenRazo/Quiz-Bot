...
class DatabaseService(UserStatsService):
    ...
    async def _create_connection_pool(self):
        ...
        try:
            ...
            self._connection_pool = await asyncpg.create_pool(
                ...
                command_timeout=self.config.connect_timeout if hasattr(self.config, 'connect_timeout') else 60,
                ssl=ssl_mode,
                server_settings={
    ...
    async def record_quiz(self, host_id: int, topic: str, category: str, difficulty: str, 
                   ...
                   is_private: bool = False, is_group: bool = False) -> int:
        """
        ...
        try:
            ...
            insert_query = """
            INSERT INTO quizzes (
                host_id, topic, category, difficulty, question_count, 
    ...
    async def get_leaderboard(self, limit: int = 10) -> List[Dict[str, Any]]:
        """
        Get global leaderboard of users.
        ...
        try:
            query = """
            SELECT user_id, username, points, level, correct_answers, wrong_answers, quizzes_taken 
            FROM users 
            ...
            logger.error(f"Failed to get leaderboard (limit {limit}): {e}")
            return []
    
    async def get_guild_leaderboard(self, guild_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        """
        Get leaderboard of users specific to a guild.
        ...
        Returns:
            List of top users in the specified guild.

        Raises:
            DatabaseError: If query fails.
        """             
        try:
            # First, let's check if we have any data in guild_members
            check_query = "SELECT COUNT(*) as count FROM guild_members WHERE guild_id = $1"
            check_result = await self._execute_query(check_query, (guild_id,), fetch_one=True)
            
            if check_result and check_result.get('count', 0) > 0:
                # We have guild member data, use the proper join
                query = """
                SELECT u.user_id, u.username, u.points, u.level, u.correct_answers, u.wrong_answers, u.quizzes_taken 
                FROM users u
                ...
                # Fall back to users who have taken quizzes that mention this guild
                # This is a temporary solution until guild_members is properly populated
                logger.info(f"No guild_members data for guild {guild_id}, falling back to quiz session data")
                query = """
                SELECT DISTINCT u.user_id, u.username, u.points, u.level, u.correct_answers, u.wrong_answers, u.quizzes_taken
                FROM users u
                INNER JOIN user_quiz_sessions s ON u.user_id = s.user_id
                WHERE s.quiz_id LIKE $1
                ORDER BY u.points DESC
                LIMIT $2
                """
                # Use a pattern that might capture guild-specific quizzes
                guild_pattern = f'%{guild_id}%'
                results = await self._execute_query(query, (guild_pattern, limit))
                return results
                
            results = await self._execute_query(query, (guild_id, limit))
            return results
        except Exception as e:
            logger.error(f"Failed to get leaderboard for guild {guild_id} (limit {limit}): {e}")
            return []
    ...
    async def _get_column_names(self, table_name):
        ...
        try:
            ...
            logger.debug(f"Detected columns in {table_name} table: {column_names}")
